  ...
  async oauthToken(
    ...
    switch (body.grant_type) {
      ...
      case GRANT_TYPES.CLIENT_CREDENTIAL: {
        ...
        await validationPipe.transform(body, null);
        const tenant: Tenant = await this.authService.validateClientCredentials(
          body.client_id,
          body.client_secret,
        );
        const token: string = await this.authService.createTechnicalAccessToken(
          tenant,
          body.scopes,
        );
        return {
      ...
      case GRANT_TYPES.REFRESH_TOKEN: {
        ...
        await validationPipe.transform(body, null);
        const { tenant, user } = await this.authService.validateRefreshToken(
          body.refresh_token,
        );
        const { accessToken, refreshToken } =
          await this.authService.createUserAccessToken(user, tenant);
