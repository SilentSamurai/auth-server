...
export class RegisterController {
    ...
    async registerDomain(
        @Headers() headers,
        @Request() request,
        @Body(new ValidationPipe(RegisterController.RegisterDomainSchema)) body: {
            name: string,
            password: string,
            email: string,
            orgName: string,
        ...
        const isPresent = await this.tenantService.existByDomain(adminContext, body.domain);
        if (isPresent) {
        ...
        const tenant = await this.tenantService.create(adminContext, body.orgName, body.domain, user);
    ...
    async signup(
        @Headers() headers,
        @Request() request,
        @Body(new ValidationPipe(RegisterController.SignUpSchema)) body: {
            name: string,
            password: string,
            email: string,
            client_id: string
        ...
        let adminContext = await this.securityService.getAdminContextForInternalUse();
        const tenant = await this.tenantService.findByClientIdOrDomain(adminContext, body.client_id);
        if (!tenant.allowSignUp) {
            throw new BadRequestException("Sign up not allowed by admin");
        }

        const existingUser = await this.usersRepository.existsBy({email: body.email});
        let user: User;
        if (!existingUser) {

            const hashedPassword = await argon2.hash(body.password);
            user = this.usersRepository.create({...body, password: hashedPassword});
            user = await this.usersRepository.save(user);
