    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.createTable(
            new Table({
                ...
                foreignKeys: [new TableForeignKey({name: "FK_clients_tenant", columnNames: ["tenant_id"], referencedTableName: "tenants", referencedColumnNames: ["id"], onDelete: "CASCADE"})],
            }),
            true,
        );

        // ── Add locked column to users ──
        await queryRunner.addColumn("users", new TableColumn({name: "locked", type: "boolean", isNullable: false, default: false}));

        // ── Add redirect_uri to auth_code ──
        await queryRunner.addColumn("auth_code", new TableColumn({name: "redirect_uri", type: "varchar", isNullable: true}));

        // ── Add binding columns to auth_code ──
        await queryRunner.addColumn("auth_code", new TableColumn({name: "client_id", type: "varchar", isNullable: false, default: "''"}));
        await queryRunner.addColumn("auth_code", new TableColumn({name: "scope", type: "varchar", isNullable: true}));
        await queryRunner.addColumn("auth_code", new TableColumn({name: "used", type: "boolean", isNullable: false, default: false}));
        await queryRunner.addColumn("auth_code", new TableColumn({name: "used_at", type: "timestamp", isNullable: true}));
        await queryRunner.addColumn("auth_code", new TableColumn({name: "expires_at", type: "timestamp", isNullable: false, default: "CURRENT_TIMESTAMP"}));

        // ── Add pkce_method_used to clients ──
        await queryRunner.addColumn("clients", new TableColumn({name: "pkce_method_used", type: "varchar", isNullable: true}));

        // ── Make auth_code PKCE columns nullable ──
        await queryRunner.changeColumn("auth_code", "code_challenge", new TableColumn({name: "code_challenge", type: "VARCHAR", isNullable: true, isUnique: false}));
        await queryRunner.changeColumn("auth_code", "method", new TableColumn({name: "method", type: "VARCHAR", isNullable: true}));
    }

    public async down(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.changeColumn("auth_code", "method", new TableColumn({name: "method", type: "VARCHAR", isNullable: false}));
        await queryRunner.changeColumn("auth_code", "code_challenge", new TableColumn({name: "code_challenge", type: "VARCHAR", isNullable: false, isUnique: true}));
