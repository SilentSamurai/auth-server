...
export class TenantBitsService {
    ...
    async addOrUpdateKey(tenantId: string, ownerTenantId: string, key: string, value: string): Promise<TenantBits> {
        ...
        let kv = await this.kvRepository.findOne({ where: { tenant: { id: tenantId }, ownerTenant: { id: ownerTenantId }, key } });
        if (kv) {
            kv.value = value;
        } else {
    ...
    async keyExists(tenantId: string, ownerTenantId: string, key: string): Promise<boolean> {
        return !!(await this.kvRepository.findOne({ where: { tenant: { id: tenantId }, ownerTenant: { id: ownerTenantId }, key } }));
    }

    async getKeyValue(tenantId: string, ownerTenantId: string, key: string): Promise<string | null> {
        const kv = await this.kvRepository.findOne({ where: { tenant: { id: tenantId }, ownerTenant: { id: ownerTenantId }, key } });
