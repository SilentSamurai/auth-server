...
export class PolicyService {
    ...
    // 1) Create a policy
    public createAuthorization(role: string, effect: string, action: string, subject: string, conditions: any = {}): Promise<Policy> {
    ...
    public deleteAuthorization(id: string): Promise<any> {
        return lastValueFrom(
            this.http.delete(`${this.baseUrl}/policy/${id}`)
        );
    }
